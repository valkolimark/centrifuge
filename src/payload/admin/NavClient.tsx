...
export default function NavClient({ counts, user }: { counts: { brands: number; newLeads: number }; user: { name: string; role: string } }) {
  ...
  return (
    <aside className="cw-nav" ref={rootRef as any}>
      <div className="brand">
        <div className="brand-icon">{/* eslint-disable-next-line @next/next/no-img-element */}<img src="/logo/cw-icon.png" alt="Centrifuge World" /></div>
        <div className="brand-word">
          <span className="cw">CENTRIFUGE</span>
          <span className="world">world</span>
          <span className="estd">Est<sup>d</sup> 1939 · Mission Control</span>
        </div>
      </div>
      ...
      <nav className="nav">
        {GROUPS.map((g) => (
          <div key={g.label}>
            ...
            {g.items.map((it) => (
              <a key={it.label} href={it.href} className={it.match(pathname) ? 'active' : undefined} title={it.label}>
                {it.icon}
                <span className="label">{it.label}</span>
                {it.pill ? <span className={`pill${it.pillCyan ? ' cyan' : ''}`}>{it.pill}</span> : null}
              </a>
            ))}
          </div>
        ))}
      ...
      <div className="profile">
        <div className="profile-row">
          <label className="avatar-wrap" title="Upload avatar">
            <span className="avatar"><span className="avatar-inner">{avatar ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={avatar} alt="User avatar" /> : initials}</span></span>
            <span className="avatar-edit">✎</span>
            <input type="file" className="avatar-input" accept="image/png,image/jpeg,image/webp" onChange={onAvatar} />
          </label>
          <div className="profile-text">
            <div className="profile-name">{user.name}</div>
            <div className="profile-role">{user.role}</div>
            <div className="profile-status"><span className="dot" />On-call · 24/7</div>
          </div>
        </div>
        <div className="avatar-hint profile-text">Click the avatar to upload a photo. PNG / JPG / WebP · max 2&nbsp;MB.</div>
      </div>
    </aside>
  )
}
